...
(function ($) {
  ...
  var loademore = function () {
    ...
    if ($(".loadmore-item-7").length > 0) {
      $(".btn-loadmore-7").on("click", function () {
        setTimeout(() => {
          $(".fl-item-7:hidden").slice(0, 2).show().css("display", "flex");
  ...
  -------------------------------------------------------------------------------------*/
  const animationtext = () => {
    const animatedTextElements = document.querySelectorAll(".text-anime-wave");
...
$(document).ready(function () {
  ...
  if (hash && $(hash).length && $(hash).hasClass("modal")) {
    let modal = new bootstrap.Modal($(hash)[0]);
    modal.show();

    $(hash).on('hidden.bs.modal', function () {
