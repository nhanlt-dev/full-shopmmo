...
if (!empty($_SESSION['userData']['role']) && in_array($_SESSION['userData']['role'], [2, 3])) {
    ...
    $detailHistories = "Người dùng <b>$nameUser</b> đã <b>$actionHistories</b> phản hồi của \"<b>$userEmail</b>\" thành công.";
    logHistory($link, $idUser, $actionHistories, $detailHistories);

    showSuccessDeleteAlert('Đã xóa liên hệ thành công!', 'Admin/contacts/listcontact/');
} else { ?>
    <script>
        Swal.fire({
            title: '403 Forbidden!',
