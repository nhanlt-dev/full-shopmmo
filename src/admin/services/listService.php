            ...
            <div class="main-content-inner style-3">
                <div class="widget-box-2 wd-listing">
                    <div class="d-flex jcsb">
                        <h3 class="title">Danh Sách Dịch Vụ</h3>
                        <div class="d-flex jcsb">
                            <?php if (!empty($_SESSION['userData']['role']) && in_array($_SESSION['userData']['role'], [2, 3])) : ?>
                                <div class="box">
                                    <a href="Admin/services/historyservice/" class="whitecolor tf-btn bg-color-primary pd-10 mr2rem"><i class="fa-regular fa-clock-rotate-left"></i> </span> Lịch Sử</a>
                                </div>
                            <?php endif; ?>
                            <div class="box">
                                <a href="Admin/services/addservice/" class="whitecolor tf-btn bg-color-primary pd-10"><span class="icon icon-plus"></span> Thêm Dịch Vụ</a>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                            ...
                            <div class="table-responsive">
                                <table id="responsive-data-table" class="table">
                                    <thead>
                                        <tr>
                                            <th class="w5 fw-6">#</th>
                                            <th class="w50 fw-6">Tên Dịch Vụ</th>
                                            <th class="w30 fw-6">Ngày Tạo</th>
                                            <th class="w15 text-center fw-6">Hành Động</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <?php
                                        $Query = mysqli_query($link, "SELECT * FROM services ORDER BY id DESC ");
                                        if (mysqli_num_rows($Query) > 0) {
                                            ...
                                            while ($row = mysqli_fetch_object($Query)) {
                                                ...
                                                $date           = date("H:i:s d/m/Y", strtotime($createdAt));
                                                $stt++;
                                                $datas[] = compact('stt', 'id', 'serviceName', 'date', 'serviceUrl');
                                            }
                                            foreach ($datas as $data) { ?>
                                                <tr class="datarow">
                                                    ...
                                                    <td>
                                                        <div>
                                                            <a href="dichvu/<?= $data['serviceUrl'] ?>/"><?= $data['serviceName'] ?></a>
                                                        </div>
                                                    </td>
                                                    <td>
                                                        <div><?= $data['date'] ?></div>
                                                    ...
                                                    <td>
                                                        <ul class="list-action">
                                                            <li class="d-flex jcsa">
                                                                <a href="Admin/services/editservice/<?= $data['id'] ?>/" class="edit-file item">
                                                                ...
                                                                <?php if (in_array($_SESSION['userData']['role'], [2, 3])) { ?>
                                                                    <a href="javascript:void(0);" class="remove-file item" onclick="showConfirmAlert('Bạn có chắc chắn muốn xóa dịch vụ này?', 'Admin/services/deleteservice/<?= $data['id'] ?>/');">
                                                                        <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
