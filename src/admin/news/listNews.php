                    ...
                    <div class="d-flex jcsb">
                        ...
                        <div class="d-flex jcsb">
                            <?php if (!empty($_SESSION['userData']['role']) && in_array($_SESSION['userData']['role'], [2, 3])) : ?>
                                <div class="box">
                                    <a href="Admin/news/historynews/" class="whitecolor tf-btn bg-color-primary pd-10 mr2rem"><i class="fa-regular fa-clock-rotate-left"></i> </span> Lịch Sử</a>
                                </div>
                            <?php endif; ?>
                            <div class="box">
                                <a href="Admin/news/addnews/" class="whitecolor tf-btn bg-color-primary pd-10"><span class="icon icon-plus"></span> Thêm Tin Tức</a>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-12">
                            <div class="table-responsive">
                                <table id="responsive-data-table" class="table">
                                    ...
                                    <tbody>
                                        <?php
                                        $newsQuery = mysqli_query($link, "SELECT * FROM news ORDER BY id DESC ");
                                        if (mysqli_num_rows($newsQuery) > 0) {
                                            ...
                                            while ($rowNews = mysqli_fetch_object($newsQuery)) {
                                                ...
                                                $newsImage  = $rowNews->newsImage;
                                                $createAt   = $rowNews->createAt;
                                                $newsUrl    = $rowNews->newsUrl;
                                                $newDate    = date("H:i:s d/m/Y", strtotime($createAt));
                                                $sttNews++;
                                                $dataNews[] = compact('sttNews', 'idNews', 'newsTitle', 'newsImage', 'newDate', 'newsUrl');
                                            }
                                            foreach ($dataNews as $news) { ?>
                                                <tr class="datarow">
                                                    ...
                                                    <td>
                                                        <div>
                                                            <a href="tintuc/<?= $news['newsUrl'] ?>/"><?= $news['newsTitle'] ?></a>
                                                        </div>
                                                    </td>
                                                    <td>
                                                        <div>
                                                            <img src="src/docs/images/imageNews/<?= $news['newsImage'] ?>" alt="<?= $news['newsTitle'] ?>" class="imagePageList">
                                                        </div>
                                                    </td>
                                                    <td>
                                                        <div><?= $news['newDate'] ?></div>
                                                    </td>
                                                    <td>
                                                        <ul class="list-action">
                                                            <li class="d-flex jcsa">
                                                                <a href="Admin/news/editnews/<?= $news['idNews'] ?>/" class="edit-file item">
                                                                ...
                                                                <?php if (in_array($_SESSION['userData']['role'], [2, 3])) { ?>
                                                                    <a href="javascript:void(0);" class="remove-file item" onclick="showConfirmAlert('Bạn có chắc chắn muốn xóa bài viết này?', 'Admin/news/deletenews/<?= $news['idNews'] ?>/');">
                                                                        <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
