                    ...
                    <div class="d-flex jcsb">
                        ...
                        <div class="d-flex jcsb">
                            <?php if (!empty($_SESSION['userData']['role']) && in_array($_SESSION['userData']['role'], [2, 3])) : ?>
                                <div class="box">
                                    <a href="Admin/pages/historypage/" class="whitecolor tf-btn bg-color-primary pd-10 mr2rem"><i class="fa-regular fa-clock-rotate-left"></i> </span> Lịch Sử</a>
                                </div>
                            <?php endif; ?>
                            <?php if (!empty($_SESSION['userData']['role']) && in_array($_SESSION['userData']['role'], [1, 2, 3])) : ?>
                                <div class="box">
                                    <a href="Admin/pages/addpage/" class="whitecolor tf-btn bg-color-primary pd-10"><span class="icon icon-plus"></span> Thêm Thương Hiệu</a>
                                </div>
                            <?php endif; ?>
                        </div>
                    </div>
                            ...
                            <div class="table-responsive">
                                <table id="responsive-data-table" class="table">
                                    <thead>
                                        <tr>
                                            ...
                                            <?php if (in_array($_SESSION['userData']['role'], [2, 3])) { ?>
                                                <th class="w15 fw-6">Chủ Sở Hữu</th>
                                            <?php } ?>
                                    ...
                                    <tbody>
                                        ...
                                        if ($roleUser == 3) {
                                            $whereCondition = "1";
                                        } elseif ($roleUser == 2 || $roleUser == 1) {
                                            $whereCondition = "p.idRepresentativePersion NOT IN (SELECT id FROM users WHERE userRole = 3)";
                                        // } elseif ($roleUser == 1) {
                                        //     $whereCondition = "p.idRepresentativePersion NOT IN (SELECT id FROM users WHERE userRole = 3 OR userRole = 0)";
                                        } else {
                                            $whereCondition = "p.idRepresentativePersion = $idUser";
                                        }
                                        ...
                                        if (mysqli_num_rows($pageQuery) > 0) {
                                            ...
                                            while ($rowPage = mysqli_fetch_object($pageQuery)) {
                                                ...
                                                $sttPage++;
                                                $dataPages[] = compact('sttPage', 'idPage', 'pageName', 'userName', 'categoryPage', 'pageImageLogo', 'pageStatus', 'pageUrl');
                                            }
                                            foreach ($dataPages as $page) { ?>
                                                <tr class="datarow">
                                                    ...
                                                    <?php if (in_array($_SESSION['userData']['role'], [2, 3])) { ?>
                                                        <td>
                                                            <div class="">
                                                                <?= ($page['pageStatus'] == 0) ? 'Tạm Chưa ' : 'Đã Cho Thuê' ?>
                                                            </div>
                                                        </td>
                                                    <?php } ?>
                                                    <td>
                                                        <ul class="list-action">
                                                            <li class="d-flex jcsa">
                                                                <a href="Admin/pages/editpage/<?= $page['idPage'] ?>/" class="edit-file item">
                                                                ...
                                                                <?php if (in_array($_SESSION['userData']['role'], [2, 3])) { ?>
                                                                    <a href="javascript:void(0);" class="remove-file item" onclick="showConfirmAlert('Bạn có chắc chắn muốn xóa bài viết này?', 'Admin/pages/deletepage/<?= $page['idPage'] ?>/');">
                                                                        <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
