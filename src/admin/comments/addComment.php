<div class="page-layout">
    ...
    if (!empty($_SESSION['userData']['role']) && in_array($_SESSION['userData']['role'], [2, 3])) :
        ...
        if (isset($_POST['addComment'])) {
            validateCsrfToken($_POST['csrf_tokenAddComment']);
            $dataComment  = [
            ...
            $insertQuery    = "INSERT INTO comments (`idUser`, `idNews`, `ratingComment`, `statusComment`, `contentComment`) VALUES (?,?,?,?,?)";
            $idComment      = executeQuery($link, $insertQuery, array_values($dataComment), true);

            $actionHistories = 'Thêm mới';
            $detailHistories = "Người dùng <b>$nameUser</b> đã <b>$actionHistories</b> bình luận bài viết id \"<b>" . $dataComment['idNewsComment'] . "</b>\" thành công.";

            logHistory($link, $idUser, $actionHistories, $detailHistories);
            showSuccessInsertAlert('Thêm mới bình luận thành công!', 'Admin/comments/listcomment/');
        } ?>
        <div class="main-content style-2 w-100">
            <div class="main-content-inner wrap-dashboard-content-2">
                <div class="widget-box-2">
                    <div class="box d-flex jcsb">
                        <h3 class="title">Thêm Mới Bình Luận</h3>
                        <div class="box">
                            <a href="Admin/comments/listcomment/" class="whitecolor tf-btn bg-color-primary pd-10"><i class="fa-duotone fa-regular fa-list"></i> Danh Sách Bình Luận</a>
                        </div>
                    </div>
                    <form class="gap-30" method="post" enctype="multipart/form-data">
                                ...
                                <div class="box box-fieldset">
                                    ...
                                    <select name="idUserComment" id="idUserComment" class="form-control formSelect">
                                        ...
                                        while ($rowUser = mysqli_fetch_array($userQuery)) {
                                            ...
                                            <option value="<?= $idUserComment ?>"><?= $nameUserComment ?></option>
                                        <?php } ?>
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="box box-fieldset">
                                    ...
                                    <select name="idNewsComment" id="idNewsComment" class="form-control formSelect">
                                        ...
                                        while ($rowNews = mysqli_fetch_array($newsQuery)) {
                                            ...
                                            <option value="<?= $idNewsComment ?>"><?= $titleNewsComment ?></option>
                                        <?php } ?>
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="box box-fieldset">
                            ...
                            <textarea class="form-control" name="contentComment" rows="4" id="contentComment"></textarea>
                        </div>
                        <div class="box-btn mt4r">
                            <input type="hidden" name="csrf_tokenAddComment" value="<?= $_SESSION['csrf_token'] ?? "" ?>">
                            <button type="submit" name="addComment" class="tf-btn style-border pd-10">Thêm Mới</button>
                        </div>
                    </form>
                </div>
            </div>
