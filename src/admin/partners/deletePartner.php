...
if (!empty($_SESSION['userData']['role']) && in_array($_SESSION['userData']['role'], [2, 3])) {
    ...
    $stmt = mysqli_prepare($link, "SELECT * FROM partners WHERE id = ?");
    mysqli_stmt_bind_param($stmt, "s", $_GET['id']);
    mysqli_stmt_execute($stmt);
    $resultCheckImage = mysqli_stmt_get_result($stmt);
    $row_resuilt = mysqli_fetch_array($resultCheckImage);

    $namePartner      = $row_resuilt['namePartner'];
    $imagePartner     = $row_resuilt['imagePartner'];
    $targetDir        = __DIR__ . "/../../../src/docs/images/imagePartners/";

    if (!empty($imagePartner) && $imagePartner !== 'defaultPartner.jpg') {
        $oldFile = $targetDir . $imagePartner;
        if (file_exists($oldFile)) {
            unlink($oldFile);
        }
    }

    $deleteQuery = "DELETE FROM partners WHERE id = ?";
    ...
    showSuccessDeleteAlert('Đã xóa bài viết thành công!', 'Admin/partners/listpartner/');
} else { ?>
    <script>
        Swal.fire({
            title: '403 Forbidden!',
