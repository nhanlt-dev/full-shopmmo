...
$count = @mysqli_num_rows(mysqli_query($link, "SELECT * FROM news"));
            ...
            <div class="row">
                <div class="col-12">
                    ...
                    <div class="grid-layout-3">
                        <?php
                        $newsQuery = mysqli_query($link, "SELECT * FROM news ORDER BY id DESC LIMIT $start, $limit");
                        if (mysqli_num_rows($newsQuery) > 0) {
                            ...
                            while ($rowNews = mysqli_fetch_object($newsQuery)) {
                                ...
                                $basePath   = "src/docs/images/imageNews/";
                                $imageName  = !empty($rowNews->newsImage) ? $rowNews->newsImage : 'defaultNews.jpg';
                                $imagePage  = $basePath . $imageName;

                                $dataNews[] = compact('idNews', 'newsTitle', 'newsUrl', 'imagePage');
                            }
                            foreach ($dataNews as $news) { ?>
                                <div class="blog-article-item style-2  ">
                                    <div class=" image-wrap  image-wrap-last-blog">
                                        <a href="tintuc/<?= $news['newsUrl'] ?>/">
                                            <img class="lazyload" data-src="<?= $news['imagePage'] ?>" src="<?= $news['imagePage'] ?>" alt="<?= $news['newsTitle'] ?>">
                                        </a>
                                    </div>
                                    <div class="article-content">
                                        ...
                                        <a href="tintuc/<?= $news['newsUrl'] ?>/" class="tf-btn-link">
                                            <span> Xem Thêm </span>
