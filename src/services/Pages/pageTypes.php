<?php
include('src/handlers/pagination/client/paginationPageTypes.php');
$url = $_GET['url'] ?? '';
$safeUrl = mysqli_real_escape_string($link, $url);
$stmt = mysqli_prepare($link, "SELECT * FROM categorypages WHERE urlCategoryPage = ?");
mysqli_stmt_bind_param($stmt, "s", $safeUrl);
...
if ($result) {
    ...
    if ($cateRow) {
        $categoryPage = $cateRow['categoryPage'] ?? "";
    }
}
...
$count = @mysqli_num_rows(mysqli_query( $link, "SELECT * FROM pages AS p
                                                            Inner Join categorypages AS cp ON cp.id = p.idCategoryPage
                                                            Where cp.urlCategoryPage = '$url'"));
            ...
            <div class="col-lg-12">
                <div class="title-inner ">
                    <ul class="breadcrumb">
                        ...
                        <li><a class="home fw-6 text-color-3" href="quangba/">Quảng Bá Doanh Nghiệp</a></li>
                        <li><?= $categoryPage ?></li>
                    </ul>
                </div>
            </div>
                ...
                <div class="col-12">
                    <div class="box-title">
                        ...
                        <div class="right">
                            ...
                            <div class="nice-select select-filter list-sort" tabindex="0">
                                <span class="current">Lọc Bởi (Mặc Định)</span>
                                <ul class="list">
                                    <li data-value="default" class="option selected">Lọc Bởi (Mặc Định)</li>
                                    <li data-value="new" class="option">Mới Nhất</li>
                                    <li data-value="old" class="option">Cũ Nhất</li>
                                </ul>
                            </div>
                        </div>
                    </div>
                    <div class="flat-animate-tab">
                        <div class="tab-content">
                            <?php
                            $pagesQuery = mysqli_query($link, "SELECT p.*, cp.urlCategoryPage
                                                                            FROM pages AS p
                                                                            INNER JOIN categorypages AS cp ON cp.id = p.idCategoryPage
                                                                            WHERE cp.urlCategoryPage = '$url'
                                                                            ORDER BY p.id DESC
                            ...
                            if (mysqli_num_rows($pagesQuery) > 0) {
                                $dataPages = [];
                                while ($rowPages = mysqli_fetch_object($pagesQuery)) {
                                    $idPage   = $rowPages->id;
                                    $pageName = $rowPages->pageName;
                                    $pageUrl  = $rowPages->pageUrl;
                                    $basePath = "src/docs/images/imageIntroduces/";
                                    $imageName = !empty($rowPages->pageImageIntroduce) ? $rowPages->pageImageIntroduce : 'defaultIntroduce.jpg';
                                    $imagePage = $basePath . $imageName;

                                    $dataPages[] = compact('idPage', 'pageName', 'pageUrl', 'imagePage');
                                }
                            ?>
                                <div class="tab-pane active show" id="gridLayout" role="tabpanel">
                                    <div class="tf-grid-layout lg-col-3 md-col-2">
                                        <?php foreach ($dataPages as $page) { ?>
                                            <div class="box-house gridLayoutBox hover-img">
                                                <div class="image-wrap">
                                                    <a href="<?= $page['pageUrl'] ?>/">
                                                        <img class="lazyload" data-src="<?= $page['imagePage'] ?>" src="<?= $page['imagePage'] ?>" alt="<?= $page['pageName'] ?>">
                                                    </a>
                                                    <div class="list-btn flex gap-8">
                                                        <a href="<?= $page['pageUrl'] ?>/" class="btn-icon find hover-tooltip">
                                                            <i class="icon-find-plus"></i>
                                                            <span class="tooltip">Xem Ngay</span>
                                                        </a>
                                                    </div>
                                                </div>
                                                <div class="content">
                                                    <h5 class="title titlePages">
                    ...
                    <div class="wrap-pagination">
                        <p class="text-1">Showing <?= ($start + 1) . '-' . min($start + $limit, $count) ?> of <?= $count ?> results.</p>
                        <ul class="wg-pagination">
                            <?= $p->pageList($currentPage, $url, $pages); ?>
                        </ul>
                    </div>
                </div>
            </div>
